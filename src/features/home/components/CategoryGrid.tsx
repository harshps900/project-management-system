                    ...
                    {categories.slice(0, 4).map((category, index) => (
                        ...
                        >
                            ...
                            <div className="absolute inset-0 p-8 flex flex-col justify-end">
                                <span className="text-white/70 text-[10px] font-bold uppercase tracking-[0.2em] mb-2">{category.count} Items</span>
                                <h3 className="text-2xl font-black text-white uppercase tracking-tight leading-none mb-4 group-hover:translate-x-2 transition-transform duration-300">
                                    {category.name}
                                </h3>
                                <Link href={`/shop?category=${category.slug}`}>
                                    <button className="bg-white text-primary px-6 py-2 text-[10px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                                        Explore
                                    </button>
                                </Link>
                            </div>
